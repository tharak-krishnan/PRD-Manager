...
class RoadmapExporter:
    ...
    def generate_pptx(self):
        ...
        for i, months_chunk in enumerate(chunks):
            self._create_timeline_slide(prs, months_chunk, i, len(chunks), features_with_dates)
    ...
    def _create_title_slide(self, prs, months):
        ...
        # Style title text
        title.text_frame.paragraphs[0].font.color.rgb = RGBColor(243, 244, 246)  # gray-100
    ...
    def _create_timeline_slide(self, prs, months_chunk, chunk_index, total_chunks, features_with_dates):
        ...
        for cat in self.categories:
            cat_features = [
                f for f in features_with_dates
                if f['category_id'] == cat.id and f['release_date'] in months_chunk
            ]
            if cat_features or any(f['category_id'] == cat.id for f in features_with_dates):
        ...
        for cat_index, category in enumerate(categories_with_features):
            ...
            for month_index, month in enumerate(months_chunk):
                ...
                feature_height = Inches(0.65) if len(month_features) <= 1 else Inches(0.4)
                feature_spacing = Inches(0.05)

                for feat_index, feature in enumerate(month_features[:3]):  # Max 3 features per cell
                    feature_top = row_top + Inches(0.1) + (feat_index * (feature_height + feature_spacing))

                    self._add_feature_card(
                        slide, feature, month_left + Inches(0.05),
                        feature_top, month_width - Inches(0.1), feature_height,
                        fill_color, text_color
                    )

    def _add_feature_card(self, slide, feature, left, top, width, height, fill_color, text_color):
        ...
        p_title.text = feature['title'][:30] + ('...' if len(feature['title']) > 30 else '')
        ...
        # Color code priority
        priority_color = self.priority_colors.get(feature['priority'], RGBColor(200, 200, 200))
...
class PRDExporter:
    ...
    def _create_summary_sheet(self, sheet):
        ...
        # Headers
        headers = ['Category', 'Description', 'Total Features', 'High Priority', 'Medium Priority', 'Low Priority']
    ...
    def _create_category_sheet(self, sheet, category):
        ...
        headers = ['ID', 'Title', 'Priority', 'Description', 'KPI', 'Customer', 
                   'Eng. Comment', 'Signoff', 'Complexity', 'Release Date']
        for col, header in enumerate(headers, start=1):
            cell = sheet.cell(row=4, column=col, value=header)
            cell.font = Font(bold=True, color='FFFFFF')
            cell.fill = PatternFill(start_color='366092', end_color='366092', fill_type='solid')
        ...
        for feature in category.features:
            ...
            sheet.cell(row=row, column=8, value='Yes' if feature.engineering_signoff else 'No')
            ...
            if feature.release_date:
                try:
                    ...
                    sheet.cell(row=row, column=10, value=release_date_obj.strftime('%B %Y'))
                except:
    ...
    def generate_word(self):
        ...
        # Metadata
        metadata = doc.add_paragraph(f'Generated: {datetime.now().strftime("%B %d, %Y at %I:%M %p")}')
    ...
    def _add_category_to_doc(self, doc, category):
        ...
        for feature in features:
            ...
            details = [
                ...
                ('Release Date', self._format_release_date(feature.release_date) if feature.release_date else '-'),
            ]
